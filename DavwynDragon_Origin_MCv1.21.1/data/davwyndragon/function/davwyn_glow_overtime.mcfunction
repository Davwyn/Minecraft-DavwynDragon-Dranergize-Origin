...
execute at @s positioned ~ ~1 ~ if entity @e[type=minecraft:marker,distance=..1.0,tag=DavwynDragonGlowEven] run setblock ~ ~ ~ minecraft:light[level=5] keep
execute at @s positioned ~ ~ ~ if entity @e[type=minecraft:marker,distance=..1.0,tag=DavwynDragonGlowOdd] run setblock ~ ~ ~ minecraft:light[level=5] keep