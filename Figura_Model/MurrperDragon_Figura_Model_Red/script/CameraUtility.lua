...
function FDCameraUpdate(CameraObj,Render,dt)
	if Render == true then
		...
		if renderer:isFirstPerson() then
			if CameraObj.HidePart ~= nil and (FDMapperObj[CameraObj.HidePart]:getOpacity() ~= 0 or CameraObj.ShaderTrace ~= ShaderActive) then
				FDMapperObj[CameraObj.HidePart]:setOpacity(0)
				if ShaderActive == false then
					FDMultiLevelPartFunction(FDMapperObj[CameraObj.HidePart],function(Part)
				...
				CameraObj.ShaderTrace = ShaderActive
			end
		else
			if FDMapperObj[CameraObj.HidePart] ~= nil and (FDMapperObj[CameraObj.HidePart]:getOpacity() ~= 1 or CameraObj.ShaderTrace ~= ShaderActive) then
				FDMapperObj[CameraObj.HidePart]:setOpacity(1)
				FDMultiLevelPartFunction(FDMapperObj[CameraObj.HidePart],function(Part)
					FDPartRenderShaderFunction(Part,true)
				end)
				CameraObj.ShaderTrace = ShaderActive
